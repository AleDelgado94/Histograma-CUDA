...
__global__ void inicializa_histograma(int num_elements, const int hist_elements, float* hist_locales, int nBloques){

	int i = (blockIdx.x * blockDim.x + threadIdx.x);


	if(i < nBloques * HISTO_ELEMENTS){

		if(threadIdx.x == 0){
			__shared__ float hist_local[HISTO_ELEMENTS];

			for(int j=0; j<hist_elements; j++)
				hist_local[j]=0.0;

			//Vector gloabal que contiene todos los histrogramas locales
			for(int k=0;k<hist_elements;k++){
				hist_locales[blockIdx.x*hist_elements+k] = hist_local[k];//hist[k];
			}
...
__global__ void histograma(float* A, float* hist, int num_elements, const int hist_elements, int nBloques){
	...
	if(i < num_elements){
		...
		atomicAdd(&(hist_local[pos]), 1.0);

		__syncthreads();


		//atomicAdd(&(hist[(blockIdx.x*HISTO_ELEMENTS) + pos]), hist_local[pos]);
		hist[(blockIdx.x*HISTO_ELEMENTS) + pos] = hist_local[pos];
		//__syncthreads();
...
__global__ void reduccion(float* hist, float* hist_reducido, int num_elements, const int hist_elements, int nBloques){
...
/**********************EJECUTANDO KERNELS***************************/
	...
	inicializa_histograma<<<BloquesPorGrid, HilosPorBloque>>>(num_elements, hist_elements, d_Histo, BloquesPorGrid);
...
/*******************COMPROBANDO RESULTADOS ****************************/
	...
	for(int j=0; j<HISTO_ELEMENTS; j++){
		//printf("%f \n", h_Reducido[j]);
		//printf("\n");
